...
class SelectInterestAdapter(private var list: List<SelectInterest>, private val doneButtonCallback: (Boolean) -> Unit): RecyclerView.Adapter<SelectInterestAdapter.SelectInterestViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SelectInterestViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_interest, parent, false)
        return SelectInterestViewHolder(view)
    }

    override fun onBindViewHolder(holder: SelectInterestViewHolder, position: Int) {
    ...
    // 데이터 업데이트 메서드 추가
    fun updateData(newList: List<SelectInterest>) {
        list = newList
        notifyDataSetChanged()
    }

    inner class SelectInterestViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: SelectInterest) {
            topicView.text = item.topic
            iconView.setImageResource(getDrawableResourceId(itemView.context, item.icon)) // 아이콘 이미지 설정
    ...
    inner class InnerAdapter(private val innerList: List<String>): RecyclerView.Adapter<InnerAdapter.InnerViewHolder>() {

        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): InnerViewHolder {
            val view = LayoutInflater.from(parent.context).inflate(R.layout.item_interest_detail, parent, false)
